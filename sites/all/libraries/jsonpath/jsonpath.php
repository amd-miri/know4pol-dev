...
class JsonPath {
  ...
  public function _callback_01($m) {
    return "[#" . (array_push($this->result, $m[1]) - 1) . "]";
  }

  public function _callback_02($m) {
  ...
  public function trace($expr, $val, $path) {
    if ($expr !== "") {
      ...
      elseif (preg_match("/^\(.*?\)$/", $loc)) {
        $this->trace($this->evalx($loc, $val) . ";" . $x, $val, $path);
      }
      // [?(expr)].
      elseif (preg_match("/^\?\(.*?\)$/", $loc)) {
  ...
  public function _callback_03($m, $l, $x, $v, $p) {
    $this->trace($m . ";" . $x, $v, $p);
  }

  public function _callback_04($m, $l, $x, $v, $p) {
    if (is_array($v[$m])) {
      $this->trace("..;" . $x, $v[$m], $p . ";" . $m);
    }
  }

  public function _callback_05($m, $l, $x, $v, $p) {
