...
function know4pol_ec_europa_preprocess_node(&$variables, $hook) {
  ...
  // Node is a file
  if($variables['type'] == 'file') _know4pol_ec_europa_preprocess_node_file($variables, $hook);
}
  
function _know4pol_ec_europa_preprocess_node_file(&$variables, $hook) {
...
/**
 * Preprocess fields field_file. Analyse legacy link content and possible more info to display into the ECL block.
 *
 * @param node $node
 * The node.
 *
...
/**
 * Provides a readable filesize text from a number by reccurently dividing by a base.
 *
 * @param int $size
 * The filesize in bytes.
 *
 * @param int $i
 * Default 0, can be used for overriding the start of the division if $size is already divided.
 *
 * @param int $base
